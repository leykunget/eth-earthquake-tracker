...
# Modify the fetch_earthquake_data function to include error handling:
fetch_earthquake_data <- function(days_ago = 30) {
  tryCatch({
    ...
    response <- GET(base_url, query = query, timeout(10))
    stop_for_status(response)  # Will error if the response is not successful
...
server <- function(input, output, session) {
  ...
  # Modify the observe block in your server function like this:
  observe({
    # Force an immediate update when the session starts
    invalidateLater(100)  # 100ms initial delay
    invalidateLater(1000 * 60 * 5)  # Then every 5 minutes
